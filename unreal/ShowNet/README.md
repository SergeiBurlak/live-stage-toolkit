...
What it does
Fixed-rate Art-Net output at up to 44 Hz, the physical maximum for DMX512, scheduled independently of frame rate.
Double-buffered channel data guarded by a critical section, so the game thread can write at any time without blocking output.
A watchdog: if the game thread stops calling KeepAlive() for longer than the configured timeout, every universe is driven to zero. A frozen engine goes to blackout rather than holding the last frame.
...
Runtime statistics: packets sent, send errors, worst loop interval, time since the last keep-alive.
Installation
