...
class ValidationError(ValueError):
    """Raised when input parameters are physically invalid, before any math runs."""


def _validate(args: argparse.Namespace) -> None:
    """
    Fail loudly and clearly on physically-invalid input, instead of letting a
    ZeroDivisionError or a silently-wrong result reach the report. These are
    the values that, left unguarded, either crash (fps, render_fps,
    compression_ratio, screen area) or produce nonsensical-but-plausible-
    looking numbers (negative width/depth/rig_height, zero performers/cameras).
    """
    checks = [
        ("--width", args.width, "must be a positive number of metres"),
        ("--depth", args.depth, "must be a positive number of metres"),
        ("--rig-height", args.rig_height, "must be a positive number of metres"),
        ("--performers", args.performers, "must be at least 1"),
        ("--cameras", args.cameras, "must be at least 1"),
        ("--fps", args.fps, "must be greater than 0"),
        ("--render-fps", args.render_fps, "must be greater than 0"),
        ("--iso", args.iso, "must be greater than 0"),
        ("--f-number", args.f_number, "must be greater than 0"),
        ("--screen-width", args.screen_width, "must be a positive number of metres"),
        ("--screen-height", args.screen_height, "must be a positive number of metres"),
        ("--projectors", args.projectors, "must be at least 1"),
        ("--projector-lumens", args.projector_lumens, "must be greater than 0"),
    ]
    errors = [f"{name} {msg} (got {value!r})" for name, value, msg in checks if value <= 0]

    if args.compressed and args.compression_ratio <= 0:
        errors.append(f"--compression-ratio must be greater than 0 when --compressed is set "
                       f"(got {args.compression_ratio!r})")

    if not (0.0 <= args.blend_overlap < 1.0):
        errors.append(f"--blend-overlap must be between 0 and 1 (got {args.blend_overlap!r}); "
                       f"it is a fraction, not a percentage")

    if args.preset not in SENSORS:
        errors.append(f"--preset {args.preset!r} is not a known sensor; "
                       f"choose one of {sorted(SENSORS)}")
...
def selftest() -> int:
    ...
    # Validation: physically-invalid input must fail loudly and cleanly,
    # never with a raw ZeroDivisionError from deep inside the math.
    bad = argparse.Namespace(**vars(ns))
    bad.fps = 0
    try:
        analyse(bad)
        raise AssertionError("expected ValidationError for fps=0")
    except ValidationError:
        pass
    except ZeroDivisionError:
        raise AssertionError("fps=0 must be caught by validation, not crash with ZeroDivisionError")
